...
extern void GetDeviceNetworkInfo(char* outBuffer, size_t maxLen, char* outIpStr);
...
void CreateMainControls(HWND hWnd, HINSTANCE hInstance) {
    // Create the top tab controller (Receive, Send, Settings tabs)
    ...
    char infoNet[512] = {0}; GetDeviceNetworkInfo(infoNet, sizeof(infoNet), NULL);
    ...
    hWndListView = CreateWindowExA(WS_EX_CLIENTEDGE, WC_LISTVIEWA, "", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 0, 0, 0, 0, hWnd, (HMENU)IDC_FILE_LISTVIEW, hInstance, NULL); ApplyWindowFont(hWndListView); ListView_SetExtendedListViewStyle(hWndListView, LVS_EX_FULLROWSELECT);
    ...
    // Extract default file/folder icons directly from system shell32.dll resources
    char sysPath[MAX_PATH]; GetSystemDirectoryA(sysPath, MAX_PATH); strcat(sysPath, "\\shell32.dll");
    HICON hIFile = NULL, hIFolder = NULL, hIText = NULL, hIPaste = NULL;
#ifdef LOCALSEND_NT
    SafeExtractIcon(sysPath, 0, 24, 24, &hIFile);
    SafeExtractIcon(sysPath, 4, 24, 24, &hIFolder);
    SafeExtractIcon(sysPath, 1, 24, 24, &hIText);
    SafeExtractIcon(sysPath, 43, 24, 24, &hIPaste);
#else
    SafeExtractIcon(sysPath, 54, 24, 24, &hIFile);
    SafeExtractIcon(sysPath, 3, 24, 24, &hIFolder);
    SafeExtractIcon(sysPath, 70, 24, 24, &hIText);
    SafeExtractIcon(sysPath, 260, 24, 24, &hIPaste);
#endif
    if (hIFile) SendMessage(hWndBtnSndFile, BM_SETIMAGE, IMAGE_ICON, (LPARAM)hIFile);
    if (hIFolder) SendMessage(hWndBtnSndFolder, BM_SETIMAGE, IMAGE_ICON, (LPARAM)hIFolder);
    if (hIText) SendMessage(hWndBtnSndText, BM_SETIMAGE, IMAGE_ICON, (LPARAM)hIText);
    if (hIPaste) SendMessage(hWndBtnSndPaste, BM_SETIMAGE, IMAGE_ICON, (LPARAM)hIPaste);
    ...
    hWndListSendFiles = CreateWindowExA(WS_EX_CLIENTEDGE, WC_LISTVIEWA, "", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 0,0,0,0, hWnd, (HMENU)IDC_LIST_SEND_FILES, hInstance, NULL); ApplyWindowFont(hWndListSendFiles); ListView_SetExtendedListViewStyle(hWndListSendFiles, LVS_EX_FULLROWSELECT);
    HIMAGELIST hSendImageList = ImageList_Create(16, 16, ILC_COLOR | ILC_MASK, 0, 1); ListView_SetImageList(hWndListSendFiles, hSendImageList, LVSIL_SMALL);
    lvc.mask = LVCF_TEXT | LVCF_WIDTH | LVCF_SUBITEM; lvc.iSubItem = 0; lvc.cx = 240; lvc.pszText = g_Lang.colFile; ListView_InsertColumn(hWndListSendFiles, 0, &lvc); lvc.iSubItem = 1; lvc.cx = 80; lvc.pszText = g_Lang.colSize; ListView_InsertColumn(hWndListSendFiles, 1, &lvc);

    hWndSendProgress = CreateWindowExA(0, PROGRESS_CLASSA, NULL, WS_CHILD, 0, 0, 0, 0, hWnd, (HMENU)IDC_SEND_PROGRESS, hInstance, NULL);
    hWndSendStatusIcon = CreateWindowExA(0, "STATIC", "", WS_CHILD | SS_ICON, 0, 0, 0, 0, hWnd, (HMENU)IDC_SEND_STATUS_ICON, hInstance, NULL);
    hWndSendStatusTxt = CreateWindowExA(0, "STATIC", "", WS_CHILD | SS_LEFT, 0, 0, 0, 0, hWnd, (HMENU)IDC_SEND_STATUS_TXT, hInstance, NULL); ApplyWindowFont(hWndSendStatusTxt);
    hWndBtnCancelSend = CreateWindowExA(0, "BUTTON", g_Lang.cancelBtn, WS_CHILD | BS_PUSHBUTTON, 0, 0, 0, 0, hWnd, (HMENU)IDC_BTN_CANCEL_SEND, hInstance, NULL); ApplyWindowFont(hWndBtnCancelSend);
    ...
    hWndListDevices = CreateWindowExA(WS_EX_CLIENTEDGE, WC_LISTVIEWA, "", WS_CHILD | LVS_REPORT | LVS_SINGLESEL, 0,0,0,0, hWnd, (HMENU)IDC_LIST_DEVICES, hInstance, NULL); ApplyWindowFont(hWndListDevices); ListView_SetExtendedListViewStyle(hWndListDevices, LVS_EX_FULLROWSELECT | LVS_EX_CHECKBOXES);
    hDeviceImageList = ImageList_Create(32, 32, ILC_COLOR | ILC_MASK, 0, 1); ListView_SetImageList(hWndListDevices, hDeviceImageList, LVSIL_SMALL);
    ...
    hWndSearchLoading = CreateWindowExA(0, PROGRESS_CLASSA, NULL, WS_CHILD, 0, 0, 0, 0, hWnd, NULL, hInstance, NULL); ShowWindow(hWndSearchLoading, SW_HIDE);
    ...
    hWndCheckRecursiveFolder = CreateWindowExA(0, "BUTTON", g_Lang.setRecursiveFolder, WS_CHILD | BS_AUTOCHECKBOX, 0, 0, 0, 0, hWnd, (HMENU)IDC_SET_RECURSIVE_FOLDER, hInstance, NULL); ApplyWindowFont(hWndCheckRecursiveFolder);
    ...
    SendMessage(hWndComboLanguage, CB_ADDSTRING, 0, (LPARAM)"Chinese (Simplified)");
    ...
    hWndRecvBtnBrowse = CreateWindowExA(0, "BUTTON", g_Lang.browse, WS_CHILD | BS_PUSHBUTTON, 0, 0, 0, 0, hWnd, (HMENU)IDC_RECV_BTN_BROWSE, hInstance, NULL); ApplyWindowFont(hWndRecvBtnBrowse);
    {
        ...
        if (hShl) {
            typedef HRESULT (WINAPI *pfnSHAutoComplete)(HWND, DWORD);
            pfnSHAutoComplete pAutoComplete = (pfnSHAutoComplete)GetProcAddress(hShl, "SHAutoComplete");
            if (pAutoComplete) pAutoComplete(hWndRecvTxtPath, 0x00000020 /* SHACF_FILESYS_DIRS */);
        }
    }
    ...
    hWndLblDiscTimeout = CreateWindowExA(0, "STATIC", g_Lang.lblDiscTimeout, WS_CHILD | SS_LEFT, 0, 0, 0, 0, hWnd, NULL, hInstance, NULL); ApplyWindowFont(hWndLblDiscTimeout);
    ...
    hWndLblMulticast = CreateWindowExA(0, "STATIC", g_Lang.lblMulticast, WS_CHILD | SS_LEFT, 0, 0, 0, 0, hWnd, NULL, hInstance, NULL); ApplyWindowFont(hWndLblMulticast);
    ...
    hWndCheckEncryption = CreateWindowExA(0, "BUTTON", g_Lang.lblEncryption, WS_CHILD | BS_AUTOCHECKBOX, 0, 0, 0, 0, hWnd, (HMENU)IDC_SET_ENCRYPTION, hInstance, NULL); ApplyWindowFont(hWndCheckEncryption);

    hWndLblDevType = CreateWindowExA(0, "STATIC", g_Lang.lblDeviceType, WS_CHILD | SS_LEFT, 0, 0, 0, 0, hWnd, NULL, hInstance, NULL); ApplyWindowFont(hWndLblDevType);
    ...
    hWndLblDevModel = CreateWindowExA(0, "STATIC", g_Lang.lblDeviceModel, WS_CHILD | SS_LEFT, 0, 0, 0, 0, hWnd, NULL, hInstance, NULL); ApplyWindowFont(hWndLblDevModel);
    ...
    hWndBtnHelp = CreateWindowExA(0, "BUTTON", g_Lang.btnHelp, WS_CHILD | BS_PUSHBUTTON, 0, 0, 0, 0, hWnd, (HMENU)IDC_SET_BTN_HELP, hInstance, NULL); ApplyWindowFont(hWndBtnHelp);
